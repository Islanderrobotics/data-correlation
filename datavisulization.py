...
class DataVisulization:
    def __init__(self, data = None,automatic = False, type_of_plot = "", column_values_for_x = None, column_values_for_y = None,alpha = None):
        self.data = data
        self.alpha = alpha
        ...
        if automatic==True:
            self.driver()
    def driver(self):
        if self.type_of_plot.upper() == "HIST":
            self.Hist()
        elif(self.x is not None and self.y is not None):
            if self.type_of_plot.upper() == "LINE":
                self.Line()
            elif self.type_of_plot.upper() == "SCATTER":
                self.Scatter()
        elif(self.x is None or self.y is None):
            raise MissingColumnNames(self.column_values_for_x and self.column_values_for_y)
        else:
            raise NotAnOpption
